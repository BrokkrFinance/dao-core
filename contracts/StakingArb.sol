    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 public initialLockedUntil;
    ...
    function initialize(address _stakingToken, uint256 _initialLockedUntil)
        external
        initializer
    {
        __Ownable_init();
        ...
        initialLockedUntil = _initialLockedUntil;
    }
    ...
    function setInitialLockedUntil(uint256 newInitialLockedUntil)
        external
        onlyOwner
    {
        require(
        ...
        initialLockedUntil = newInitialLockedUntil;
    }
    ...
    function stakeOnBehalf(address onBehalf, uint256 amount)
        public
        nonReentrant
        whenNotPaused
        updateReward(onBehalf)
    {
        ...
        _balances[onBehalf] = _balances[onBehalf].add(amount);
        stakingToken.safeTransferFrom(msg.sender, address(this), amount);
        emit Staked(msg.sender, onBehalf, amount);
    }

    function stake(uint256 amount) external {
        stakeOnBehalf(msg.sender, amount);
    }
    ...
    function exit() external passedInitialLockedUntil {
